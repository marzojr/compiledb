...
cc_compile_regex = re.compile(r"^.*-?[gq]?cc-?[0-9.]*(\.exe)?$|^.*-?(clang-?[0-9.]*-)?(cl)?(\.exe)?$")
cpp_compile_regex = re.compile(r"^.*-?[gcq]\+\+-?[0-9.]*(\.exe)?$|^.*-?clang\+\+-?[0-9.]*$|^.*-?(clang-[0-9.]*-)?(cl)?(\.exe)?$")
file_regex = re.compile(r"^.+\.c$|^.+\.cc$|^.+\.m$|^.+\.mm$|^.+\.cpp$|^.+\.cxx$|^.+\.cu$|^.+\.s$", re.IGNORECASE)
